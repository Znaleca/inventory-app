...
<div class="p-6 border-b border-sky-100 bg-white flex items-center justify-between shrink-0">
    <div>
        <p class="font-mono text-[10px] font-bold uppercase tracking-widest text-sky-500 mb-1">Inventory://Categories</p>
        <h3 class="text-xl font-black text-[#0f172a] tracking-tight">Category Registry</h3>
    </div>
    <span class="rounded-lg bg-sky-50 px-3 py-1.5 text-[11px] font-bold text-sky-600">{{ $categories->count() }} records</span>
</div>
...
<div class="bg-white relative overflow-hidden">
    ...
    <div class="overflow-x-auto">
        ...
        {{-- Group header --}}
        <div class="flex items-center gap-2 px-5 py-2.5 border-b border-dashed border-slate-100 bg-sky-50/50 ml-1">
            <span class="h-2 w-2 inline-block {{ $group['color'] === 'violet' ? 'bg-violet-500' : 'bg-sky-500' }}"></span>
            <span class="text-[10px] font-mono font-bold uppercase tracking-widest {{ $group['color'] === 'violet' ? 'text-violet-600' : 'text-sky-600' }}">
                {{ $group['label'] }}
            </span>
        ...
        <table class="min-w-full text-sm">
            <thead>
                <tr class="border-b border-slate-100 bg-sky-50/80">
            ...
            <tbody class="divide-y divide-sky-50">
                ...
                <tr class="hover:bg-sky-50 transition-colors">
                    <td class="pl-5 pr-3 py-3">
                        <span class="text-sm font-bold text-[#0f172a]">{{ $category->name }}</span>
                    </td>
                    <td class="px-3 py-3">
                        <span class="text-xs font-mono text-sky-500 line-clamp-1 max-w-[280px]">{{ $category->description ?: '—' }}</span>
                    </td>
                    <td class="whitespace-nowrap px-3 py-3 text-center">
                        <span class="font-mono text-[11px] font-bold text-slate-600 bg-sky-50 border border-sky-100 px-2.5 py-1.5">
    ...
    <div class="flex flex-col items-center justify-center py-20 text-center ml-1">
        ...
        <p class="text-sm font-semibold text-sky-500 mt-1">No categories yet. Create the first one.</p>
        <a href="{{ route('categories.create') }}"
            class="mt-6 inline-flex items-center gap-2 bg-sky-500 hover:bg-sky-600 text-white px-5 py-2.5 text-[11px] font-mono font-bold uppercase tracking-widest transition-colors border border-blue-700">
            + New Category
        </a>
    </div>
    @endif
