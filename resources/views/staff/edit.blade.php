...
@section('content')
    <div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
        ...
        <div class="p-6 border-b border-sky-100 flex items-center justify-between shrink-0 mb-6">
            <div>
                <p class="font-mono text-[10px] font-bold uppercase tracking-widest text-sky-500 mb-1">Staff://Edit</p>
                <h3 class="text-xl font-black text-[#0f172a] tracking-tight">Edit Staff Member</h3>
                <p class="text-xs text-slate-400 font-mono mt-1">Update details for {{ $staff->display_name ?? $staff->name }}.</p>
            </div>
        </div>
        ...
        <form action="{{ route('staff.update', $staff) }}" method="POST">
            ...
            <div class="bg-white border border-sky-100 relative mb-4 overflow-hidden">
                ...
                <div class="">
                    <div class="px-5 py-4 border-b border-sky-100">
                        <div class="flex items-center gap-2">
                            ...
                            <p class="text-[10px] font-mono font-bold text-sky-500 uppercase tracking-widest">02 // Role
                                Details</p>
                        </div>
                    </div>
                    <div class="px-5 py-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <select id="type" name="type" required
                                class="block w-full border border-sky-100 bg-sky-50 focus:bg-white focus:border-sky-500 focus:outline-none py-2.5 px-3 text-sm text-[#0f172a] transition-colors">
                                <option value="programmer" {{ old('type', $staff->type) === 'programmer' ? 'selected' : '' }}>Programmer</option>
                                <option value="tech support" {{ old('type', $staff->type) === 'tech support' ? 'selected' : '' }}>Tech Support</option>
                                <option value="supervisor" {{ old('type', $staff->type) === 'supervisor' ? 'selected' : '' }}>Supervisor</option>
                                <option value="head" {{ old('type', $staff->type) === 'head' ? 'selected' : '' }}>Head</option>
                            </select>
