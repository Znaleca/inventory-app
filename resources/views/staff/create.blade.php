...
<div class="mx-auto max-w-2xl">
    ...
    <div class="mb-5">
        ...
        <p class="text-xs text-slate-400 font-mono mt-0.5">Add a doctor, nurse, or technician to the directory.</p>
    </div>
            ...
            <div class="ml-1">
                <div class="px-5 py-4 border-b border-slate-100">
                    <div class="flex items-center gap-2">
                        ...
                        <p class="text-[10px] font-mono font-bold text-violet-600 uppercase tracking-widest">01 // Personal Information</p>
                    </div>
                </div>
                <div class="px-5 py-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div class="sm:col-span-1">
                        <label for="title" class="block text-sm font-bold text-slate-700 mb-1.5">Title <span class="font-normal text-slate-400">(Optional)</span></label>
                        ...
                        @error('title') <p class="mt-1 text-xs font-mono font-bold text-rose-500">{{ $message }}</p> @enderror
                    </div>
                    <div class="sm:col-span-2">
                        <label for="name" class="block text-sm font-bold text-slate-700 mb-1.5">Full Name <span class="text-rose-500">*</span></label>
                        <input type="text" id="name" name="name" value="{{ old('name') }}" required placeholder="e.g. Maria Santos"
                            class="block w-full border border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-500 focus:outline-none py-2.5 px-3 text-sm text-slate-800 transition-colors">
            ...
            <div class="ml-1">
                <div class="px-5 py-4 border-b border-slate-100">
                    <div class="flex items-center gap-2">
                        ...
                        <p class="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-widest">02 // Role Details</p>
                    </div>
                </div>
                <div class="px-5 py-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label for="type" class="block text-sm font-bold text-slate-700 mb-1.5">Type <span class="text-rose-500">*</span></label>
                        <select id="type" name="type" required
                            class="block w-full border border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-500 focus:outline-none py-2.5 px-3 text-sm text-slate-800 transition-colors">
                            <option value="doctor" {{ old('type') === 'doctor' ? 'selected' : '' }}>Doctor</option>
                            <option value="nurse" {{ old('type') === 'nurse' ? 'selected' : '' }}>Nurse</option>
                            <option value="technician" {{ old('type') === 'technician' ? 'selected' : '' }}>Technician</option>
                            <option value="other" {{ old('type') === 'other' ? 'selected' : '' }}>Other</option>
                        </select>
                        @error('type') <p class="mt-1 text-xs font-mono font-bold text-rose-500">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="specialization" class="block text-sm font-bold text-slate-700 mb-1.5">Specialization</label>
                        <input type="text" id="specialization" name="specialization" value="{{ old('specialization') }}" placeholder="e.g. Cardiology"
                            class="block w-full border border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-500 focus:outline-none py-2.5 px-3 text-sm text-slate-800 transition-colors">
                        @error('specialization') <p class="mt-1 text-xs font-mono font-bold text-rose-500">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
        </div>
