...
<body class="min-h-full antialiased text-slate-800 overflow-hidden"
    ...
    <div class="relative flex flex-col h-full w-full overflow-hidden rounded-r-[1.25rem] backdrop-blur-xl ring-1 ring-white/10"
        ...
        <div class="relative flex flex-col h-full w-full z-10">
            ...
            {{-- Branding Header --}}
            <div class="flex items-center shrink-0 px-6 h-[80px] border-b border-white/10 bg-white/[0.02] relative justify-center lg:justify-start">
    ...
    <a href="{{ route('dashboard') }}"
        ...
        <div x-show="sidebarExpanded"
            ...
            <span class="text-[10px] font-mono font-bold text-sky-400/90 uppercase tracking-[0.4em] mb-1">
                BGHMC System
            </span>
                ...
                @foreach($navLinks as $link)
                    ...
                    <a href="{{ $targetUrl }}"
                        ...
                        <div x-show="!sidebarExpanded"
                            ...
                            {{ $link['label'] }}
                            <div class="absolute -left-1 top-1/2 -translate-y-1/2 w-2 h-2 bg-white border-l border-b border-sky-100 rotate-45"></div>
                        </div>

                        {{-- Active Indicator (White Glow) --}}
                        @if($isActive)
                            <div class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-3/4 bg-white rounded-r-full shadow-[0_0_12px_rgba(255,255,255,0.8)]"></div>
                        @endif

                        {{-- Icon Wrapper --}}
                        <div class="relative z-10 flex items-center justify-center p-2 rounded-lg transition-transform duration-300 group-hover:scale-110
                            ...
                            <svg class="shrink-0 w-5 h-5 transition-colors duration-300"
                                fill="none" viewBox="0 0 24 24" stroke-width="{{ $isActive ? '2.5' : '2' }}" stroke="currentColor">
                                {!! $link['icon'] !!}
                            </svg>
                        </div>

                        {{-- Text Label --}}
                        <span class="text-[14px] font-medium tracking-wide whitespace-nowrap transition-all duration-300
                ...
                @if(Auth::user() && Auth::user()->role === 'admin')
                    ...
                    @foreach($adminLinks as $link)
                        ...
                        <a href="{{ $targetUrl }}"
                            ...
                            @if($isActive)
                                <div class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-3/4 bg-white rounded-r-full shadow-[0_0_12px_rgba(255,255,255,0.9)]"></div>
                            @endif
    ...
    <button @click="sidebarExpanded = !sidebarExpanded"
        ...
        <div class="absolute inset-0 rounded-full bg-sky-400/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
...
<style>
    ...
    <header class="relative z-30 flex h-[75px] items-center justify-between gap-4 px-8 bg-white/80 backdrop-blur-md border-b border-sky-100 shrink-0 shadow-sm">
        ...
        <div class="absolute top-0 left-0 w-full h-[3px] bg-gradient-to-r from-[#0f172a] via-[#0ea5e9] to-transparent"></div>
        ...
        <div class="flex items-center gap-6">
            ...
            <div class="hidden sm:flex items-center gap-3 pl-2" aria-hidden="true">
                ...
                <div class="relative flex h-8 w-8 items-center justify-center">
                    <div class="absolute inset-0 rounded-full bg-sky-100 opacity-40 animate-pulse"></div>
                    <div class="absolute inset-1 rounded-full border border-sky-300/60 border-dashed animate-[spin_8s_linear_infinite]"></div>
                    <div class="h-2.5 w-2.5 rounded-full bg-gradient-to-tr from-sky-500 to-blue-600 shadow-[0_0_10px_rgba(14,165,233,0.6)] relative z-10"></div>
                </div>
                
                {{-- Tracing Path --}}
                <div class="flex items-center">
                    <div class="h-[2px] w-12 bg-gradient-to-r from-sky-400 to-sky-100 rounded-full relative overflow-hidden">
                        <div class="absolute top-0 left-0 w-4 h-full bg-white opacity-80 blur-[1px] animate-[scan-sweep_2.5s_linear_infinite]"></div>
                    </div>
                    <div class="h-[4px] w-[4px] rotate-45 bg-sky-200 ml-1"></div>
                    <div class="h-[2px] w-4 bg-sky-100 ml-1"></div>
                </div>
    ...
    <span class="relative flex h-2 w-2">
        ...
        <div class="flex items-center gap-6 shrink-0">
            ...
            <div class="hidden md:flex items-center gap-3 px-4 py-2 bg-sky-50/50 border border-sky-100 rounded-full">
                <span class="relative flex h-2 w-2">
                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-sky-400 opacity-75"></span>
                    <span class="relative inline-flex rounded-full h-2 w-2 bg-sky-500"></span>
                </span>
                <span class="text-[11px] font-bold text-sky-700 uppercase tracking-wider">Node Active</span>
            </div>
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" @click.outside="open = false"
                    ...
                    {{-- Avatar --}}
                    <div class="h-10 w-10 bg-[#0f172a] rounded-xl flex items-center justify-center text-white font-bold text-sm shadow-md group-hover:bg-[#1e3a8a] transition-colors border-2 border-white ring-1 ring-sky-100">
                        {{ strtoupper(substr(Auth::user()->name ?? 'A', 0, 1)) }}
                    </div>

                    <div class="hidden lg:flex flex-col items-start leading-tight">
                        <span class="text-[14px] font-extrabold text-[#0f172a] group-hover:text-sky-600 transition-colors">
                    ...
                    <svg viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 text-sky-300 transition-transform duration-300" :class="{ 'rotate-180': open }">
                ...
                <div x-show="open" 
                    ...
                    <div class="px-5 py-4 bg-sky-50/30 border-b border-sky-50">
                        <p class="text-[10px] font-bold text-sky-400 uppercase tracking-[0.2em]">User Account</p>
                    </div>
