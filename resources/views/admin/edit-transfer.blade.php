...
<div>
    ...
    <div class="mb-5">
        <p class="text-[10px] font-mono font-semibold text-amber-600 uppercase tracking-[0.25em] mb-1">Admin://Transfers//Edit</p>
        <h1 class="text-xl font-bold text-slate-800 tracking-tight">Edit Transfer Record</h1>
        <p class="text-xs text-slate-400 font-mono mt-0.5">Item: <span class="font-bold text-slate-600">{{ $transfer->item->name ?? '—' }}</span></p>
    </div>
            ...
            <form action="{{ route('admin.transfers.update', $transfer) }}" method="POST" class="px-5 py-5 space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $transfer->quantity) }}" min="0"
                            class="block w-full border border-slate-200 bg-slate-50 focus:bg-white focus:border-amber-500 focus:outline-none py-2.5 px-3 text-sm font-mono text-slate-800 transition-colors">
                    ...
                    <div>
                        ...
                        <input type="text" name="destination" id="destination" value="{{ old('destination', $transfer->destination) }}"
                            class="block w-full border border-slate-200 bg-slate-50 focus:bg-white focus:border-amber-500 focus:outline-none py-2.5 px-3 text-sm text-slate-800 transition-colors">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="serial_number" id="serial_number" value="{{ old('serial_number', $transfer->serial_number === 'N/A' ? '' : $transfer->serial_number) }}"
                            class="block w-full border border-slate-200 bg-slate-50 focus:bg-white focus:border-amber-500 focus:outline-none py-2.5 px-3 text-sm font-mono text-slate-800 transition-colors" placeholder="Leave blank for N/A">
                        @endif
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="block w-full border border-slate-200 bg-slate-50 focus:bg-white focus:border-amber-500 focus:outline-none py-2.5 px-3 text-sm text-slate-800 transition-colors">{{ old('notes', $transfer->notes) }}</textarea>
