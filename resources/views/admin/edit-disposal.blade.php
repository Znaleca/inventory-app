...
<div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
    ...
    <div class="p-6 border-b border-sky-100 flex items-center justify-between shrink-0 mb-6">
        <div>
            <p class="font-mono text-[10px] font-bold uppercase tracking-widest text-sky-500 mb-1">Admin://Disposals//Edit</p>
            <h3 class="text-xl font-black text-[#0f172a] tracking-tight">Edit Disposal Record</h3>
        </div>
    </div>
            ...
            <form action="{{ route('admin.disposals.update', $disposal) }}" method="POST" class="px-5 py-5 space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $disposal->quantity) }}" min="0" @if(optional($disposal->item)->item_type === 'device') readonly @endif
                            class="block w-full border border-sky-100 {{ optional($disposal->item)->item_type === 'device' ? 'bg-sky-50 cursor-not-allowed' : 'bg-slate-50 focus:bg-white focus:border-slate-500 focus:outline-none' }} py-2.5 px-3 text-sm font-mono text-[#0f172a] transition-colors">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select name="stock_entry_id" id="stock_entry_id" class="block w-full border border-sky-100 bg-slate-50 focus:bg-white focus:border-slate-500 focus:outline-none py-2.5 px-3 text-sm font-mono text-[#0f172a] transition-colors">
                    ...
                    <div>
                        ...
                        <input type="text" name="disposed_by" id="disposed_by" value="{{ old('disposed_by', $disposal->disposed_by) }}"
                            class="block w-full border border-sky-100 bg-slate-50 focus:bg-white focus:border-slate-500 focus:outline-none py-2.5 px-3 text-sm text-[#0f172a] transition-colors">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="date" name="disposed_at" id="disposed_at" value="{{ old('disposed_at', $disposal->disposed_at?->format('Y-m-d')) }}"
                            class="block w-full border border-sky-100 bg-slate-50 focus:bg-white focus:border-slate-500 focus:outline-none py-2.5 px-3 text-sm font-mono text-[#0f172a] transition-colors">
                ...
                <div>
                    ...
                    <textarea name="reason" id="reason" rows="3"
                        class="block w-full border border-sky-100 bg-slate-50 focus:bg-white focus:border-slate-500 focus:outline-none py-2.5 px-3 text-sm text-[#0f172a] transition-colors">{{ old('reason', $disposal->reason) }}</textarea>
