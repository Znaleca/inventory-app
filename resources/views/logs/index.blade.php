...
<div x-data="{ activeTab: 'stock-entries', search: '' }">
    ...
    @php
        $tabs = [
            ...
            ['id' => 'borrows',      'label' => 'Borrows',        'count' => $catCounts['Borrows'],      'active' => 'border-sky-500 text-sky-600 bg-sky-50'],
            ...
            ['id' => 'disposals',    'label' => 'Disposals',      'count' => $catCounts['Disposals'],    'active' => 'border-slate-600 text-slate-700 bg-sky-100'],
            ['id' => 'items',        'label' => 'Items',          'count' => $catCounts['Items'],        'active' => 'border-violet-500 text-violet-700 bg-violet-50'],
        ];
    @endphp
    
    <div class="flex items-center justify-between mb-3 border-b border-sky-100 pb-2">
        <div class="flex flex-wrap gap-1.5 ">
            @foreach($tabs as $tab)
            <button @click="activeTab = '{{ $tab['id'] }}'"
                :class="activeTab === '{{ $tab['id'] }}' ? '{{ $tab['active'] }}' : 'bg-white text-sky-500 hover:bg-sky-50'"
                class="inline-flex items-center gap-2 border border-sky-100 px-4 py-2 text-[10px] font-mono font-bold uppercase tracking-widest transition-colors rounded-t-lg">
                {{ $tab['label'] }}
        ...
        <div class="relative w-64 hidden md:block">
            ...
            <input type="text" x-model="search" placeholder="Search item in tab..." class="w-full border border-sky-100 bg-white pl-9 pr-4 py-1.5 text-xs font-mono text-slate-700 placeholder-slate-400 focus:outline-none focus:border-slate-400 transition-colors">
        </div>
    </div>


    {{-- ══ STOCK ENTRIES TAB ══ --}}
    <div x-show="activeTab === 'stock-entries'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" style="display:none;" class="mb-4">
        <div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr class="hover:bg-sky-50 transition-colors" x-show="search === '' || '{{ strtolower($entry->item->name ?? '') }}'.includes(search.toLowerCase())">
                        ...
                        <td class="whitespace-nowrap px-6 py-4 font-mono text-xs text-sky-500">
                        ...
                        <td class="whitespace-nowrap px-6 py-4 text-xs text-sky-500">{{ $entry->expiry_date?->format('M d, Y') ?? '—' }}</td>
                        <td class="whitespace-nowrap px-6 py-4 text-xs text-sky-500">{{ $entry->received_date?->format('M d, Y') ?? '—' }}</td>
                        <td class="px-6 py-4 text-xs text-sky-500 max-w-[200px] truncate" title="{{ $entry->notes }}">{{ $entry->notes ?: '—' }}</td>
                    </tr>
                    @empty
    ...
    {{-- ══ USAGE LOGS TAB ══ --}}
    <div x-show="activeTab === 'usage-logs'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" style="display:none;" class="mb-4">
        <div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
            <div class="px-6 py-4 border-b border-sky-100">
                <div class="flex items-center gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr class="hover:bg-sky-50 transition-colors" x-show="search === '' || '{{ strtolower($log->item->name ?? '') }}'.includes(search.toLowerCase())">
                        ...
                        <td class="whitespace-nowrap px-6 py-4 text-xs text-sky-500">{{ $log->used_at?->format('M d, Y') ?? '—' }}</td>
                        <td class="px-6 py-4 text-xs text-sky-500 max-w-[200px] truncate" title="{{ $log->notes }}">{{ $log->notes ?: '—' }}</td>
                    </tr>
                    @empty
    ...
    {{-- ══ BORROWS TAB ══ --}}
    <div x-show="activeTab === 'borrows'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" style="display:none;" class="mb-4">
        <div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
            <div class="px-6 py-4 border-b border-sky-100">
                <div class="flex items-center gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr class="hover:bg-sky-50 transition-colors" x-show="search === '' || '{{ strtolower($borrow->item->name ?? '') }}'.includes(search.toLowerCase())">
                        ...
                        <td class="whitespace-nowrap px-6 py-4">
                            <span class="font-mono font-black text-sky-500" title="Borrowed">{{ $borrow->quantity_borrowed }}</span> / 
                        ...
                        <td class="whitespace-nowrap px-6 py-4">
                            ...
                            <span class="inline-flex items-center gap-1.5 border border-blue-200 bg-sky-50 px-2 py-0.5 text-[9px] font-mono font-bold tracking-widest uppercase text-sky-600">Partial</span>
                            @else
                        ...
                        <td class="whitespace-nowrap px-6 py-4 font-mono text-xs text-sky-500">{{ $borrow->borrowed_at?->format('M d, Y') ?? '—' }}</td>
                        <td class="px-6 py-4 text-xs text-sky-500 max-w-[200px] truncate" title="{{ $borrow->notes }}">{{ $borrow->notes ?: '—' }}</td>
                    </tr>
                    @empty
    ...
    {{-- ══ RETURNS TAB ══ --}}
    <div x-show="activeTab === 'returns'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" style="display:none;" class="mb-4">
        <div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
            <div class="px-6 py-4 border-b border-sky-100">
                <div class="flex items-center gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr class="hover:bg-sky-50 transition-colors" x-show="search === '' || '{{ strtolower($return->item->name ?? '') }}'.includes(search.toLowerCase())">
                        ...
                        <td class="px-6 py-4 font-bold text-[#0f172a]">{{ $return->item->name ?? '—' }}</td>
                        <td class="whitespace-nowrap px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-xs text-sky-500 max-w-[200px] truncate" title="{{ $return->notes }}">{{ $return->notes ?: '—' }}</td>
                    </tr>
                    @empty
    ...
    {{-- ══ TRANSFERS TAB ══ --}}
    <div x-show="activeTab === 'transfers'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" style="display:none;" class="mb-4">
        <div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
            <div class="px-6 py-4 border-b border-sky-100">
                <div class="flex items-center gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr class="hover:bg-sky-50 transition-colors" x-show="search === '' || '{{ strtolower($transfer->item->name ?? '') }}'.includes(search.toLowerCase())">
                        ...
                        <td class="whitespace-nowrap px-6 py-4 text-xs text-sky-500">{{ $transfer->transferred_at?->format('M d, Y') ?? '—' }}</td>
                        <td class="px-6 py-4 text-xs text-sky-500 max-w-[200px] truncate" title="{{ $transfer->notes }}">{{ $transfer->notes ?: '—' }}</td>
                    </tr>
                    @empty
    ...
    {{-- ══ DISPOSALS TAB ══ --}}
    <div x-show="activeTab === 'disposals'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" style="display:none;" class="mb-4">
        <div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
            <div class="px-6 py-4 border-b border-sky-100">
                <div class="flex items-center gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr class="hover:bg-sky-50 transition-colors" x-show="search === '' || '{{ strtolower($disposal->item->name ?? '') }}'.includes(search.toLowerCase())">
                        ...
                        <td class="whitespace-nowrap px-6 py-4 text-xs text-sky-500">{{ $disposal->disposed_at?->format('M d, Y') ?? '—' }}</td>
                    </tr>
                    @empty
    ...
    {{-- ══ ITEMS TAB ══ --}}
    <div x-show="activeTab === 'items'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" style="display:none;" class="mb-4">
        <div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
            <div class="px-6 py-4 border-b border-sky-100">
                <div class="flex items-center gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr class="hover:bg-sky-50 transition-colors" x-show="search === '' || '{{ strtolower($item->name) }}'.includes(search.toLowerCase())">
