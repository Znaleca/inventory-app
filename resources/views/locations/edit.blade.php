...
@section('content')
    <div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
        ...
        <div class="p-6 border-b border-sky-100 bg-white flex items-center justify-between shrink-0 mb-6">
            <div>
                <p class="font-mono text-[10px] font-bold uppercase tracking-widest text-sky-500 mb-1">Locations://Edit</p>
                <h3 class="text-xl font-black text-[#0f172a] tracking-tight">Edit Location</h3>
                <p class="text-xs text-slate-400 font-mono mt-1">Modifying <strong class="text-slate-600">{{ $location->name }}</strong></p>
            </div>
        </div>
                    ...
                    <div class="space-y-4 mt-1">
                        <div>
                            ...
                            <input type="text" value="{{ ucfirst($location->type) }} (Read Only)" disabled
                                class="block w-full border border-sky-100 bg-sky-50/50 py-2.5 px-3 text-sm font-mono text-sky-500 cursor-not-allowed">
