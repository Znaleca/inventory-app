...
<div class="p-6 border-b border-sky-100 bg-white flex items-center justify-between shrink-0">
    <div>
        <p class="font-mono text-[10px] font-bold uppercase tracking-widest text-sky-500 mb-1">Inventory://Locations</p>
        <h3 class="text-xl font-black text-[#0f172a] tracking-tight">Storage Locations</h3>
    </div>
    <span class="rounded-lg bg-sky-50 px-3 py-1.5 text-[11px] font-bold text-sky-600">{{ $storages->count() + $sections->count() }} locations</span>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    {{-- Storages Column --}}
    <div class="bg-white border border-sky-100 relative overflow-hidden flex flex-col h-full">
        <div class="absolute top-0 left-0 w-1 h-full bg-emerald-500"></div>
        
        <div class="px-5 py-4 border-b border-dashed border-slate-100 bg-sky-50/50 ml-1">
            <div class="flex items-center justify-between">
                ...
                <span class="font-mono text-[10px] font-bold text-slate-400 bg-sky-50/50 border border-sky-100 px-2 py-0.5">
                    {{ $storages->count() }}
                </span>
            </div>
        </div>
            ...
            <tbody class="divide-y divide-sky-50">
                ...
                <tr class="hover:bg-sky-50 transition-colors">
                    <td class="px-5 py-3.5">
                        <span class="text-sm font-bold font-mono text-[#0f172a] tracking-tight">{{ $loc->name }}</span>
                    </td>
        ...
        <div class="flex-1 flex flex-col items-center justify-center py-12 text-center opacity-70 ml-1">
            ...
            <p class="text-xs font-semibold text-sky-500">No storage locations</p>
        </div>
        @endif
    </div>
    ...
    <div class="bg-white border border-sky-100 relative overflow-hidden flex flex-col h-full">
        ...
        <div class="px-5 py-4 border-b border-dashed border-slate-100 bg-sky-50/50 ml-1">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-[10px] font-mono font-bold text-sky-600 uppercase tracking-widest">Sections / Bins</h3>
                </div>
                <span class="font-mono text-[10px] font-bold text-slate-400 bg-sky-50/50 border border-sky-100 px-2 py-0.5">
                    {{ $sections->count() }}
                </span>
        ...
        <div class="flex-1 flex flex-col items-center justify-center py-12 text-center opacity-70 ml-1">
            <p class="font-mono text-[10px] text-slate-400 uppercase tracking-widest mb-1">// Empty</p>
            <p class="text-xs font-semibold text-sky-500">No sections defined</p>
        </div>
        @endif
    </div>
