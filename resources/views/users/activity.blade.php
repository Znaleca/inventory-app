...
@section('actions')
    <a href="{{ route('users.index') }}"
        class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-600 shadow-sm hover:bg-slate-50 transition-all">
...
<div class="mx-auto max-w-5xl space-y-6">
    ...
    {{-- Hero Profile Card --}}
    <div class="relative overflow-hidden rounded-[2rem] bg-white border border-slate-200/60 shadow-[0_8px_30px_-12px_rgba(0,0,0,0.08)]">

        {{-- Cover Gradient --}}
        <div class="h-40 w-full relative border-b border-slate-700/20"
            ...
            <svg class="absolute inset-0 h-full w-full opacity-10" xmlns="http://www.w3.org/2000/svg">
                <defs><pattern id="admin-pattern" width="40" height="40" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r="2" fill="currentColor"/></pattern></defs>
                <rect width="100%" height="100%" fill="url(#admin-pattern)" class="text-white"/>
            </svg>
            {{-- Admin badge --}}
            <div class="absolute top-4 right-6">
                <span class="inline-flex items-center gap-1.5 px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full
                    {{ $user->role === 'admin' ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 'bg-white/10 text-white/70 border border-white/20' }}">
                    <span class="w-1.5 h-1.5 rounded-full {{ $user->role === 'admin' ? 'bg-indigo-400' : 'bg-white/50' }}"></span>
                    {{ ucfirst($user->role ?? 'user') }}
                </span>
            </div>
        </div>

        <div class="px-8 sm:px-12 pb-8 relative">
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 -mt-14 mb-6">
                <div class="relative rounded-full p-1.5 bg-white shadow-xl w-fit">
                    <div class="flex h-24 w-24 items-center justify-center rounded-full text-3xl font-black text-white shadow-inner
                        {{ $user->role === 'admin' ? 'bg-gradient-to-tr from-indigo-600 to-violet-500' : 'bg-gradient-to-tr from-slate-600 to-slate-500' }}">
            ...
            <div class="flex flex-wrap items-center gap-3 mt-3">
                ...
                @if(!$user->is_active)
                <span class="px-3 py-1 text-[11px] font-bold text-rose-600 bg-rose-50 border border-rose-200 rounded-full tracking-widest uppercase">
                    Deactivated
                </span>
                @endif
            </div>
    ...
    <div x-data="{ activeFilter: 'All' }"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-8 py-6 border-b border-slate-100 bg-slate-50/50">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-slate-900">Activity Log — {{ $user->name }}</h3>
                    <p class="text-xs font-medium text-slate-500 mt-0.5">All recorded actions for this account</p>
                </div>
            </div>
            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold bg-slate-100 text-slate-600 border border-slate-200 shrink-0">
            ...
            <div
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span class="px-2 py-0.5 text-[10px] font-black uppercase tracking-widest rounded-md {{ $badge }}">{{ $type }}</span>
                    </div>
                    @if(!empty($tx['detail']))
                ...
                <div class="text-right shrink-0">
                    ...
                    <p class="text-[10px] text-slate-300 font-medium mt-0.5">{{ $date->format('M d, Y') }}</p>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div class="flex flex-col items-center justify-center py-16 px-6 text-center">
            ...
            <h3 class="text-sm font-bold text-slate-900">No Activity Found</h3>
            <p class="text-xs text-slate-500 mt-1 max-w-sm mx-auto">This account has no recorded activity in the system yet.</p>
        </div>
        @endif
    </div>
