...
@section('content')
    <div class="bg-white rounded-2xl overflow-hidden border border-sky-100">
        ...
        <div class="p-6 border-b border-sky-100 bg-white flex items-center justify-between shrink-0 mb-6">
            <div>
                <p class="font-mono text-[10px] font-bold uppercase tracking-widest text-sky-500 mb-1">Stock://Receive</p>
                <h3 class="text-xl font-black text-[#0f172a] tracking-tight">Receive Incoming Stock</h3>
                <p class="text-xs text-slate-400 font-mono mt-1">Add new "{{ $item->name }}" to inventory. Current stock: <span class="text-indigo-600 font-bold">{{ $item->total_stock }} {{ $item->unit }}</span></p>
            </div>
        </div>
        ...
        <form action="{{ route('stock.store', $item) }}" method="POST" id="stock-form" x-data="{ qty: {{ old('quantity', 1) }} }">
            ...
            {{-- ============================== --}}
            <div id="batch-details-container" class="bg-white border border-sky-100 mb-4 relative">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mt-1">
                        @if($item->item_type === 'device')
                            <div class="batch-field sm:col-span-2">
                                ...
                                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 max-h-72 overflow-y-auto border border-sky-100">
                                    <template x-for="i in (qty > 0 ? qty : 1)" :key="i">
                                        <div class="relative p-3">
                                            ...
                                            <input type="text" name="serial_numbers[]"
                                                class="block w-full border border-sky-100 bg-sky-50 focus:bg-white focus:border-blue-500 focus:outline-none py-2.5 pl-9 pr-3 text-sm font-mono text-[#0f172a] transition-colors"
                        ...
                        @if($item->item_type === 'consumable')
                            <div x-data="stockEntryForm()">
                                <div class="batch-field sm:col-span-2 bg-indigo-50 border border-indigo-200 p-3 mb-3">
                            ...
                            <div class="batch-field">
                                ...
                                <input type="date" name="expiry_date" x-model="expiryDate"
                                    class="block w-full border border-sky-100 bg-sky-50 focus:bg-white focus:border-blue-500 focus:outline-none py-2.5 px-3 text-sm font-mono text-[#0f172a] transition-colors" required>
